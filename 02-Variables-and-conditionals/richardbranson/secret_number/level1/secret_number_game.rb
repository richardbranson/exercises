...
class SecretNumberGame
  ...
  def play
    greeting
    @guesses.times do |x|
      guesses_left = guesses_to_i -= 1 #Find a way to decrement guesses_left each time through the loop.
  ...
  def greet(player_name)
     puts "Welcome #{player_name} you have 3 guesses to guess the correct number." #Create a string to use as your greeting. It should include the player_name and the rules, with the number of guesses the player gets.
  end

  def created_by
    puts "Richard Branson" #Created_by is a string that includes your first and last name
  end
